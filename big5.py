...
import streamlit.components.v1 as components  # 新增：用於執行置頂腳本
...
def process_recommendation(df, user_id, a, manual_cat, selected_city):
    ...
    for cid, count, lbl in plan:
        ...
        for _, r in sorted_pool.iterrows():
            recs.append({
                "排名": rank, "依據": lbl, "景點名稱": r['景點名稱'],
                "縣市": r['縣市'], "星級": f"⭐ {r['Star']}", "評論數": r['評論數']
            })
            seen.add(r['景點名稱'])
            rank += 1
...
def main():
    ...
    if st.session_state.step == 1:
        ...
        if st.button("🚀 生成推薦清單", type="primary", use_container_width=True):
        ...
        with result_area:
            ...
            st.markdown("---")
            st.info("💡 **看完推薦後，請向下滑動填寫回饋問卷，謝謝！**")
            st.markdown("<br><br>", unsafe_allow_html=True) # 增加物理間距
