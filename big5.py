...
def save_feedback(scores, text):
    ...
    data = {
        ...
        "E分數": f"{p['E']:.2f}", "A分數": f"{p['A']:.2f}", "C分數": f"{p['C']:.2f}",
        "N分數": f"{p['N']:.2f}", "O分數": f"{p['O']:.2f}",
        "PU1": scores["PU1"], "PU2": scores["PU2"], "PU3": scores["PU3"],
        "US1": scores["US1"], "US2": scores["US2"], "US3": scores["US3"],
        "其他建議": text,
        "推薦清單": "|".join([r['name'] for r in st.session_state.recs])
    }

    f_path = 'recommendation_log_academic.csv'
    df_new = pd.DataFrame([data])
    df_new.to_csv(f_path, mode='a', index=False, header=not os.path.exists(f_path), encoding='utf-8-sig')
...
def main():
    ...
    df = load_data()
    if df is None:
        st.error("❌ 找不到資料檔 `TAIWAN_FILTERED.csv`，請確認檔案已放在同一資料夾中。")
        return
    ...
    if st.session_state.step == 1:
        ...
        with st.expander("📊 資料庫診斷"):
            ...
            st.dataframe(df['縣市'].value_counts(), use_container_width=True)

        st.info(f"🆔 使用者編號：**{st.session_state.user_id}**")

        selected_city = st.selectbox("您想去哪個縣市？", VALID_CITIES)
        cat_options = {
            "F1": "F1 - 腎上腺素活動", "F2": "F2 - 荒野自然活動", "F3": "F3 - 派對、音樂與夜生活", 
            "F4": "F4 - 陽光、水與沙灘", "F5": "F5 - 博物館、船遊與觀景點", "F6": "F6 - 主題與動物公園",
            "F7": "F7 - 文化遺產", "F8": "F8 - 運動與競賽", "F9": "F9 - 美食活動", 
            "F10": "F10 - 健康與福祉", "F11": "F11 - 自然現象"
        }
        manual_cat = st.selectbox("感興趣的類型：", list(cat_options.keys()), format_func=lambda x: cat_options[x])

        st.subheader("人格特質測驗")
        questions = [
            {'id': 'q1', 'text': '1. 趨向於安靜、少言。'}, {'id': 'q2', 'text': '2. 富有同情心、溫柔的人。'},
            {'id': 'q3', 'text': '3. 傾向於雜亂無章。'}, {'id': 'q4', 'text': '4. 處事冷靜、能很好地處理壓力。'},
            {'id': 'q5', 'text': '5. 對藝術、美學沒什麼興趣。'}, {'id': 'q6', 'text': '6. 很有活力。'},
            {'id': 'q7', 'text': '7. 有時對人無理。'}, {'id': 'q8', 'text': '8. 能堅持到任務完成。'},
            {'id': 'q9', 'text': '9. 常感到情緒低落、憂鬱。'}, {'id': 'q10', 'text': '10. 有豐富的想像力。'},
            {'id': 'q11', 'text': '11. 害羞、內斂。'}, {'id': 'q12', 'text': '12. 待人禮貌、體貼。'},
            {'id': 'q13', 'text': '13. 做事有效率、能完成計畫。'}, {'id': 'q14', 'text': '14. 容易感到焦慮。'},
            {'id': 'q15', 'text': '15. 對事物有很多好奇心。'},
        ]
        ...
        with st.form("feedback_form"):
            ...
            if st.form_submit_button("送出回饋"):
                save_feedback({"PU1": pu1, "PU2": 5, "PU3": 5, "US1": us1, "US2": 5, "US3": 5}, other_text)
